...
export default function AISuite() {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="space-y-8 max-w-7xl mx-auto p-4 lg:p-6 text-white min-h-[calc(100vh-80px)]"
    >
      {/* Immersive Header */}
      <motion.div 
      ...
      >
        <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-bl from-primary/30 via-purple-500/10 to-transparent rounded-full blur-3xl -z-10 pointer-events-none"></div>
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] font-black text-primary uppercase tracking-[0.2em] bg-primary/10 py-1.5 px-3 rounded-full border border-primary/20">TaskNest Intelligence</span>
            </div>
            <h1 className="text-4xl font-black tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-white to-white/60">
              AI Execution Suite
            </h1>
            <p className="text-sm text-slate-400 max-w-2xl font-light leading-relaxed">
              A premium, cohesive dashboard merging specialized AI models into a single unified workspace. Experience real-time analytics, dynamic visualizations, and direct system integrations powered by state-of-the-art Generative AI.
            </p>
          </div>
        </div>
      </motion.div>
              ...
              return (
                <motion.button
                  ...
                  className={`w-full text-left p-4 rounded-2xl border transition-all duration-300 flex items-start gap-4 ${
                    isSelected
                      ? 'bg-gradient-to-r from-slate-800/80 to-slate-900/50 border-primary/40 shadow-lg shadow-primary/10 translate-x-2'
                ...
                >
                  ...
                  <div className="min-w-0">
                    <span className={`font-bold text-sm block leading-tight ${isSelected ? 'text-white' : 'text-slate-300'}`}>{proj.name}</span>
        ...
        <div className="lg:col-span-8 space-y-8">
          <AnimatePresence mode="wait">
            ...
            >
              <div className="border-b border-white/10 pb-6">
                <span className="text-xs font-black text-primary tracking-[0.15em] uppercase block">Parameter Configuration</span>
                <h2 className="text-2xl font-black text-white mt-1">{activeProject.name}</h2>
                <p className="text-sm text-slate-400 mt-2 leading-relaxed font-light">{activeProject.description}</p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="pt-4">
                <motion.button
                  ...
                  type="submit"
                  disabled={loading}
                  className="w-full bg-gradient-to-r from-primary to-purple-600 hover:from-primary/90 hover:to-purple-500 disabled:from-slate-800 disabled:to-slate-900 text-white font-bold py-4 px-6 rounded-2xl shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:shadow-[0_0_30px_rgba(79,70,229,0.5)] transition-all flex items-center justify-center gap-2 tracking-[0.1em] uppercase text-xs"
                >
                  {loading ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" /> Initializing Neural Engine...
                    </>
                  ) : (
                    <>
                      <Cpu className="w-5 h-5" /> Run Intelligent Analysis
                    </>
                  )}
                </motion.button>
              </div>
          ...
          {loading && (
            <motion.div 
              ...
              className="glass-panel p-12 text-center flex flex-col items-center justify-center gap-6 min-h-[300px]"
            >
              ...
              <div className="space-y-1.5">
                <h4 className="font-bold text-white tracking-wide text-lg">Synthesizing Neural Context</h4>
                <p className="text-sm text-slate-400 font-light">Running algorithmic analysis and processing results...</p>
              </div>
            </motion.div>
          )}
          </AnimatePresence>
          ...
          {result && !loading && (
            <motion.div 
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              className="glass-panel p-6 md:p-8 space-y-6 relative overflow-hidden"
            >
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[10px] font-black text-primary uppercase tracking-[0.2em] block animate-pulse">Execution Complete</span>
                    <h3 className="font-black text-white text-xl">Diagnostic Report</h3>
                  </div>
                </div>
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  ...
                  className="text-xs font-bold text-white/50 hover:text-white transition-all uppercase tracking-[0.1em] bg-white/5 hover:bg-white/10 py-2 px-4 rounded-xl border border-white/10 flex items-center gap-2"
                >
